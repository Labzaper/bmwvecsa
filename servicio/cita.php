<?php 
  ...
  if(isset($_POST['send'])) {
	  $to = 'guillermo.cruces@bmwvecsa.com, servicio.puebla@bmwvecsa.com';
	  $subject = 'Solicitud de cita de servicio';
	  // list expected fields
	  $expected = array('nombre', 'email', 'telefono', 'chasis', 'modelo', 'kilometraje', 'comments');
	  // set required fields
	  ...
	  if($mailSent){
		  header('Location: http://bmwvecsa.com/servicio/gracias.php');
		  exit;
	  }
  }
?>
...
<head>
	...
	<title>Cita de Servicio - BMW Vecsa</title>
	...
	<div class="container">
		...
		<div class="content">
			...
			<section class="datos-agencia">				
				<div class="titulo-seccion">Cita se servicio</div>
				...
				<form id="contacto" method="post" action="" accept-charset="UTF-8">
					...
					<div class="envelope-icon">
						<label class="icon-input" for="kilometraje"><i class="icon-meter"></i></label>
						<input placeholder="Kilometraje" name="kilometraje" id="kilometraje" type="text"
							<?php if($missing || $errors){
								echo 'value="' . htmlentities($kilometraje, ENT_COMPAT, 'UTF-8') . '"';
							}?>				
						>
							<?php if($missing && in_array('kilometraje', $missing)) { ?>
							<br />
							<span class="warning">Favor de introducir el kilometraje.</span>
							<?php } ?>					
					</div>
